...
public class ActionWithNumber {
    ...
    public static int calculateSummaryOfDigitsOfNumber(int number) throws NotNaturalNumberException {
        checkNaturalNumber(number);//check input
    ...
    //exception method (checking whether it is natural number)
    private static void checkNaturalNumber(int number) throws NotNaturalNumberException {
        if (number < MIN_NATURAL_NUMBER){
            throw new NotNaturalNumberException(" isn't a natural number!", number);
        }
    }

    //exception method (trying to calculate 0^exponent)
    private static void checkZero(double number) throws ZeroBaseException {
        if (number == 0.0){
            throw new ZeroBaseException("Exception! Trying to calculate 0^exponent");
        }
    }

    //exception method (trying to raise to a negative number)
    private static void checkExponent(int exponent) throws NegativeNumberException {
        if (exponent < 0){
